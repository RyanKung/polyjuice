use crate::models::{
    AnnualReportResponse, ContentStyleResponse, DomainStatusResponse,
    EngagementResponse, FollowerGrowthResponse, TemporalActivityResponse,
};
...
pub fn convert_annual_report_response(api_data: serde_json::Value) -> Result<AnnualReportResponse, String> {
    ...
    let display_name = user.get("display_name").and_then(|v| v.as_str()).map(|s| s.to_string());
    let year = api_data.get("year")
        .and_then(|v| v.as_i64())
        .map(|v| v as i32)
        .unwrap_or(2025);
    ...
    let social_growth = api_data.get("social_growth").ok_or("Missing 'social_growth' field")?;
    let current_followers = social_growth.get("current_followers")
        .and_then(|v| v.as_u64())
        .ok_or("Missing or invalid 'current_followers' in social_growth")? as usize;
    let current_following = social_growth.get("current_following")
        .and_then(|v| v.as_u64())
        .ok_or("Missing or invalid 'current_following' in social_growth")? as usize;
    let followers_at_start = social_growth.get("followers_at_start")
        .and_then(|v| v.as_u64())
        .ok_or("Missing or invalid 'followers_at_start' in social_growth")? as usize;
    let following_at_start = social_growth.get("following_at_start")
        .and_then(|v| v.as_u64())
        .ok_or("Missing or invalid 'following_at_start' in social_growth")? as usize;
    let net_growth = social_growth.get("net_growth")
        .and_then(|v| v.as_i64())
        .ok_or("Missing or invalid 'net_growth' in social_growth")?;
    let monthly_snapshots = social_growth.get("monthly_snapshots")
    ...
    // Parse engagement
    let engagement = api_data.get("engagement").ok_or("Missing 'engagement' field")?;
    ...
    let content_style_raw = api_data.get("content_style").ok_or("Missing 'content_style' field")?;
    
    // Parse top_emojis
    let top_emojis = content_style_raw.get("top_emojis")
        .ok_or("Missing 'top_emojis' in content_style")?
        .clone();
    
    // Parse top_words and calculate percentage
    let top_words_raw = content_style_raw.get("top_words")
    ...
    if let Some(avg_cast_length) = content_style_raw.get("avg_cast_length").and_then(|v| v.as_f64()) {
        content_style_json["avg_cast_length"] = serde_json::json!(avg_cast_length);
    }
    if let Some(total_characters) = content_style_raw.get("total_characters").and_then(|v| v.as_u64()) {
        content_style_json["total_characters"] = serde_json::json!(total_characters);
    }
    if let Some(frames_used) = content_style_raw.get("frames_used").and_then(|v| v.as_u64()) {
        content_style_json["frames_used"] = serde_json::json!(frames_used);
    }
    if let Some(frames_created) = content_style_raw.get("frames_created").and_then(|v| v.as_u64()) {
        content_style_json["frames_created"] = serde_json::json!(frames_created);
    }
    if let Some(channels_created) = content_style_raw.get("channels_created").and_then(|v| v.as_u64()) {
